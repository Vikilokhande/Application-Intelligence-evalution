...
SAMPLE_RATE = 24000  # 24 kHz mono

async def generate_segment_audios(temp_dir: Path):
    ...
    for idx, seg in enumerate(SCRIPT_SEGMENTS):
        mp3_file = temp_dir / f"seg_{idx:03d}.mp3"
        wav_file = temp_dir / f"seg_{idx:03d}.wav"
        
        # Generate MP3
        comm = edge_tts.Communicate(seg["text"], voice=voice, rate="+0%")
        await comm.save(str(mp3_file))
        
        # Convert MP3 to standardized 24kHz Mono 16-bit PCM WAV
        cmd = [
            ffmpeg_exe, "-y",
            "-i", str(mp3_file),
            "-ar", str(SAMPLE_RATE),
        ...
        print(f"Segment {idx+1:02d}/35 [{seg['start_time']} - {seg['end_time']}] - {len(audio_array)/SAMPLE_RATE:.2f}s audio generated.")
...
def build_full_voiceover():
    ...
    temp_dir = project_root / "temp_full_tts"
    master_wav = project_root / "master_audio_track.wav"

    if not input_video.exists():
        print(f"Error: {input_video} not found")
        return

    # Total duration: 551.84 seconds
    total_duration_sec = 551.84
    total_samples = int(total_duration_sec * SAMPLE_RATE) + SAMPLE_RATE
    master_buffer = np.zeros(total_samples, dtype=np.float32)

    print("Step 1: Generating all 35 TTS audio segments...")
    wav_files = asyncio.run(generate_segment_audios(temp_dir))

    print("\nStep 2: Placing segments onto the master 9m 11s audio timeline...")
    for seg in wav_files:
        start_idx = int(seg["start_sec"] * SAMPLE_RATE)
        seg_samples = seg["samples"].astype(np.float32)
        end_idx = start_idx + len(seg_samples)
        
        if end_idx > total_samples:
            seg_samples = seg_samples[:total_samples - start_idx]
            end_idx = total_samples
        
        # Place into master buffer with soft gain
        master_buffer[start_idx:end_idx] += seg_samples

    # Normalize audio to prevent clipping and ensure optimal volume
    max_val = np.max(np.abs(master_buffer))
    if max_val > 0:
        target_max = 28000.0  # 85% of full scale
        master_buffer = (master_buffer / max_val) * target_max

    master_int16 = np.clip(master_buffer, -32767, 32767).astype(np.int16)

    # Write full master WAV
    ...
    print(f"Master WAV written: {master_wav} ({master_wav.stat().st_size / (1024*1024):.2f} MB, {len(master_int16)/SAMPLE_RATE:.2f}s)")

    # Step 3: Multiplex master WAV onto input video
    print("\nStep 3: Muxing master audio into video (preserving 100% video quality)...")
    ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()
    ...
    # Step 4: Write TXT documentation
    print("\nStep 4: Writing DECC_REVIEW_PORTAL_voiceover_script.txt...")
    txt_lines = [
        "==================================================",
        "DECC REVIEW PORTAL — OFFICIAL SYNCHRONIZED VOICEOVER SCRIPT",
        "==================================================",
        "Target Video: DECC_REVIEW_PORTAL_voiceover.mp4",
        "Total Duration: 09:11.84 (551.84 seconds)",
        "Voice: en-IN-PrabhatNeural (Calm Indian-English Professional Voice)",
        "Principle: 'AI assists. Human decides.'",
        "Total Synchronized Segments: 35",
        "==================================================",
        ""
    ]
    ...
    print("\n==================================================")
    print("SUCCESS: ENTIRE SCRIPT (ALL 35 SEGMENTS) SYNCHRONIZED INTO VIDEO")
    print(f"Video File: {output_video} ({output_video.stat().st_size / (1024*1024):.2f} MB)")
    print(f"Script File: {script_txt}")
    print("==================================================")

if __name__ == "__main__":
    build_full_voiceover()
